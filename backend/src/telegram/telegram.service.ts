...
export class TelegramService implements OnModuleInit, OnModuleDestroy {
  ...
  async promptForBirthday(telegramId: string, language: string) {
    if (!telegramId || telegramId.startsWith('guest-') || isNaN(Number(telegramId))) return;
    try {
      const text = language === 'uz'
        ? `🎉 Birinchi buyurtmangiz muvaffaqiyatli rasmiylashtirildi, tashakkur!\n\nBiz kelajakda Maxsus Chegirmalar berishimiz uchun, iltimos *Tug'ilgan sanangizni* quyidagi shaklda xabar qilib yozib yuboring:\nKun.Oy.Yil (Misol: 13.01.2005)`
        : `🎉 Ваш первый заказ успешно оформлен, спасибо!\n\nДля получения специальных скидок в будущем, отправьте нам вашу *Дату рождения* в следующем формате:\nДень.Месяц.Год (Пример: 13.01.2005)`;
      await this.bot.telegram.sendMessage(telegramId, text, { parse_mode: 'Markdown' });
    } catch (e) {
      this.logger.error(`Failed to send birthday prompt to ${telegramId}`, e);
    }
  }
}
